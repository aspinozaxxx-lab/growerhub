...
Серийная подготовка формирует `device_id` вида `GROVIKA_XXXXXX` из последних трёх байт eFuse MAC, получает одноразовый пароль через административный API и записывает `/cfg/mqtt.json` в LittleFS. Локальная Wi-Fi точка доступа называется ровно как `device_id`, без дополнительного префикса. Пароль не показывается в web UI и логах. Без корректной конфигурации MQTTS не запускается; web UI показывает `device_id`, endpoint и диагностическое состояние подключения.
...
Обновление отклоняется при работающем насосе. Устройство публикует этапы
`downloading` и `restarting` либо машинный код ошибки, затем перезапускается.

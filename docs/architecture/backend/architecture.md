# Backend

Backend построен вокруг доменов. Адаптеры REST и MQTT принимают внешние сообщения, выполняют технический маппинг и вызывают публичные операции доменных Facade.

## Пакеты

- `api` - REST controllers, REST DTO и маппинг ошибок.
- `mqtt` - MQTT-издатель, подписчик, обработчики, шлюзы и модели сообщений.
- `common` - стабильные общие контракты, утилиты и конфигурационный wiring.
- `config` - общая конфигурация приложения.
- `diagnostics` - техническая диагностика.
- `llm` - внешний шлюз для advisor.
- доменные пакеты - `advisor`, `auth`, `automation`, `demo`, `device`, `firmware`, `journal`, `maintenance`, `onboarding`, `plant`, `pump`, `sensor`, `user`, `zigbee`.

## Домены

Фактические домены backend:

- `advisor` - рекомендации по поливу.
- `auth` - аутентификация, токены, методы входа и SSO.
- `automation` - помещения, боксы, привязки ресурсов и сценарии автоматизации.
- `device` - устройства, shadow, ACK и service events.
- `demo` - изолированные песочницы, начальное наполнение и симуляция оборудования по ADR-006.
- `firmware` - хранение и запуск обновлений прошивки.
- `journal` - журнал растений и детали полива.
- `maintenance` - технические фоновые сценарии обслуживания исторических данных.
- `onboarding` - состояние и постоянное завершение первичной настройки пользователя.
- `plant` - растения, жизненный цикл и метрики.
- `pump` - насосы, привязки и ручной полив.
- `sensor` - датчики, привязки, текущие показания и история.
- `user` - пользователи, профиль и администрирование.
- `zigbee` - пользовательские координаторы, изолированные snapshot Zigbee2MQTT и командный шлюз для Zigbee-устройств.

Описание каждого домена находится в `docs/architecture/backend/domains/`.

## Границы

У каждого домена есть один публичный Facade в корне доменного пакета. Внешний код обращается к домену через Facade и типы из `contract`.

Внутренние пакеты `engine` и `jpa` принадлежат только своему домену. Другие домены и адаптеры не импортируют чужие `engine` и `jpa`.

Домен не импортирует `api` и `mqtt`. Адаптеры не импортируют JPA entity и repository.

`common` не содержит JPA entity и не владеет бизнес-данными.

MQTT-адаптер сохраняет признак RETAIN при передаче сообщения от подписчика обработчику. Повтор сохранённого native `state` при подписке остаётся в техническом журнале, но не поступает в `DeviceFacade`: время фактического измерения неизвестно, поэтому он не обновляет связь, shadow, историю и полив. До нового живого state используется ранее сохранённое состояние из БД. Обработка snapshot Zigbee остаётся самостоятельным маршрутом.

Обезличенная продуктовая воронка является композитным административным REST-ответом. `ProductAnalyticsController` агрегирует только множества и счётчики из публичных Facade доменов `user`, `zigbee` и `automation` согласно ADR-001; персональные данные и внутренние идентификаторы в ответ не входят.

...
Обычные workers исключают технических DEMO-владельцев из каталога. Отдельный worker demo вызывает те же расчёты сценариев и полива для одного владельца. При записи ресурсов проверяется совпадение режима владельца и устройства; квоты демо сериализуются блокировкой его владельца (ADR-006).

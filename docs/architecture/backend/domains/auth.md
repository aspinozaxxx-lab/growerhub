...
Auth не владеет профилем пользователя. Секреты, TTL, cookie settings и provider URL должны приходить из конфигурации. Парсинг токена не должен открывать доступ к данным без проверки пользователя.

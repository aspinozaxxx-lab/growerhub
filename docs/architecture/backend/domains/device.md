...
- `seedSimulatedHistory(String deviceId, Map<LocalDateTime, DeviceShadowState> history)` — порция истории только DEMO/SIMULATED через sensor и plant, без изменения shadow, статусов датчиков и обработки насосов.
...
Домен владеет устройствами, хешем device token, временем MQTT-подготовки, аппаратным профилем firmware, состоянием ограничений привязки, последним state, ACK и service events. Датчики, насосы и растения не являются его данными; при обработке state домен делегирует запись показаний, историю растений и полив соответствующим доменам.
...
Facade принимает state, ack и events от адаптеров, обновляет device records и shadow, вызывает нужные домены для насосов, датчиков и растений. Для Grovika в measurements допускается только soil-канал 0; legacy-каналы с другим номером игнорируются. Для OTA домен сохраняет `hw_profile`, целевую версию, URL, состояние, ошибку, correlation ID и время; ACK принимается только для текущего correlation ID, а успех определяется по `fw_ver` нового state. Административная подготовка атомарно создаёт непривязанное устройство и отдельный Dynamic Security client с фиксированным client ID и изолированным namespace. При старте backend сверяет ACL ранее подготовленных устройств через `modifyRole` без смены паролей. Backend сохраняет только SHA-256 и один раз возвращает открытый пароль. Повторная подготовка требует явной ротации; удаление устройства отзывает broker credentials.
...
execution_kind неизменяем: PHYSICAL для существующих устройств, SIMULATED только при внутреннем создании DemoFacade. Физический ingress, credentials, claim и OTA отклоняют виртуальное устройство; claim проверяет тип после блокировки строки. Виртуальная телеметрия поступает через отдельные методы Facade (ADR-006).

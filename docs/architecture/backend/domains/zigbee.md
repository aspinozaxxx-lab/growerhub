...
MQTT adapter классифицирует legacy и пользовательские topics и передаёт
identity координатора со snapshot-сообщением в Facade. Facade всегда обновляет
текущий raw JSON snapshot. В history записываются изменения дискретных
свойств, каждое явное событие и числовые значения по интервалу либо при
значимом изменении; служебные настройки остаются только в snapshot. Интервалы,
пороги и списки свойств задаются конфигурацией, checkpoint хранится в snapshot.
Maintenance удаляет старые служебные значения, оставляет числовую точку в час,
дискретные переходы и все события. Числовые графики и последовательности
дискретных переходов имеют отдельные конфигурируемые лимиты ответа, чтобы
частые переключения не искажали расчёт длительности состояния. REST проверяет
владельца и публикует команды только в base topic через gateway.
...
SIMULATED-координатор создаётся внутри DemoFacade без broker credentials. Обычный MQTT ingress отвергает его; виртуальные snapshot проходят общий разбор, историю и статистику через отдельный метод Facade. Команды маршрутизируются по неизменяемому типу координатора; создание физических координаторов, ротация и permit join для демо запрещены (ADR-006).

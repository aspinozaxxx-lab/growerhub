...
function bindingOptionValue(binding) {
  if (!binding?.source_type) return '';
  return optionValue({
    source_type: binding.source_type,
    native_sensor_id: binding.native_sensor_id || null,
    native_pump_id: binding.native_pump_id || null,
    zigbee_ieee_address: binding.zigbee_ieee_address || null,
    zigbee_property: binding.zigbee_property || binding.command_property || null,
    command_property: binding.command_property || null,
    on_value: binding.on_value || null,
    off_value: binding.off_value || null,
  });
}
...
function optionsForRole(role, catalog) {
  ...
  if (role === 'AC_SWITCH' || role === 'EXHAUST_SWITCH' || role === 'LIGHT_SWITCH') {
    zigbeeDevices
...
function resourcePayload(role, value) {
  const parsed = parseOptionValue(value);
  if (!parsed) return null;
  const isSwitch = role === 'AC_SWITCH' || role === 'EXHAUST_SWITCH' || role === 'LIGHT_SWITCH';
  return {
    role,
...
function AdminAutomation() {
  ...
  const renderResources = (scopeType, scopeId, roles) => {
    const key = scopeKey(scopeType, scopeId);
    return (
      <div className="admin-automation-grid">
        {roles.map((role) => {
          const options = optionsForRole(role, catalog);
          return (
            <label className="admin-automation-field" key={role}>
              <span>{ROLE_LABELS[role] || role}</span>
              <select
                className="admin-select"
                value={resourceDrafts[key]?.[role] || ''}
        ...
        return (
          <Surface variant="card" padding="md" className="admin-section" key={room.id}>
            ...
            <div className="admin-automation-block">
              ...
              {renderResources(ROOM_SCOPE, room.id, ROOM_RESOURCE_ROLES)}
            </div>
                ...
                {listOrEmpty(room.boxes).map((box) => {
                  ...
                  return (
                    <div className="admin-automation-box" key={box.id}>
                      ...
                      <div className="admin-automation-block">
                        <h4>Ресурсы бокса</h4>
                        {renderResources(BOX_SCOPE, box.id, BOX_RESOURCE_ROLES)}
                      </div>

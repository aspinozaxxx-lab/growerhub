...
## Привлечение и активация: база на 17 сентября 2026 года

Окно сравнения — 20 августа–16 сентября, без текущего неполного дня:

- Search Console: 1485 показов и 31 клик, из них 20 кликов с мобильных устройств.
- GA4: 35 поисковых сессий и 29 пользователей на главной, в статьях, каталоге
  и инструкциях подключения. Входы в `/app` сюда не включены.
- Главная поисковая точка входа — статья про автополив в Home Assistant:
  64 показа и 6 кликов в Google; в GA4 — 8 поисковых сессий на эту статью.
- Яндекс: 35 страниц в поиске, активных диагностических проблем нет;
  sitemap содержит 142 URL и загружен без ошибок 14 сентября.
- Метрика: 116 визитов, из них часть относится к кабинету владельца.
- Все четыре пользователя с событиями демо в GA4 приходятся на 12–13 сентября,
  дни проверки релиза. Запусков демо из Organic Search в этом окне нет.
  Эти четыре пользователя не считаются доказанными новыми клиентами.

17 сентября в счётчике Метрики созданы девять отсутствовавших JS-целей:
`login_view`, `sso_start` и все семь `demo_*` из списка аудита. У всех точное
совпадение идентификатора; существующие цели сохранены. Исторические нули до
создания целей нельзя трактовать как отсутствие действий.

В GA4 зарегистрированы event-scoped определения `Farm mode` (`mode`),
`CTA placement` (`placement`), `Locale` (`locale`) и `Demo action` (`action`).
Отчёты по этим параметрам начинают наполняться после регистрации определений;
исторические значения до 17 сентября не восстанавливаются задним числом.

Проверяемый путь: публичный вход → `demo_open` → `demo_ready` → `demo_action`
→ `demo_save` → `demo_real_setup_start` → реальное `first_device_seen`.
Запуск демо, сохранение и подключение оборудования — разные результаты.

Входы по `view=overview|automations|watering|farm|plants` открывают нужный раздел
после создания изолированной сессии. Параметр `action` события `demo_open`
содержит выбранный раздел, `placement` — место ссылки на сайте. Неизвестное
значение `view` ведёт на обзор; публичная ссылка не выполняет команд устройств.

Ссылки публикаций используют `utm_source=telegram|vk|instagram|github`,
`utm_medium=social|referral` и `utm_campaign=demo_launch`. Проверки production
отмечаются `utm_source=qa&utm_medium=internal&utm_campaign=growth_validation_20260917`
и исключаются из оценки привлечения. UTM не ставятся на внутренние ссылки сайта.

Следующий срез должен отдельно показать внешние источники, посетителей демо
и их первые действия. Изменение CTR оценивается после накопления новых показов;
несколько визитов не доказывают преимущество варианта страницы. Приоритет
контента — уже найденные темы Zigbee2MQTT, подключения датчика и Home Assistant.

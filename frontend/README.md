...
Сборка завершается ошибкой, если отсутствует хотя бы один из 56 переводов статьи
или нарушена связь `translation_of`. Она формирует 71 русский и 71 английский
...
Снимки настоящего интерфейса на виртуальной ферме для обеих локалей.
Нужен изолированный backend без MQTT с включённым демо, proxy `/api`
и сборка с `VITE_DEMO_ENABLED=true`. Скрипт разрешает только localhost:
...
Live-проверка обходит 142 URL, внутренние ссылки, матрицу перенаправлений,
...
`step`, `connection_mode`, тип сценария, `locale`, `mode`, `action` и `provider`. Email, user ID, coordinator
...
Состояние на 16 августа 2026 года:
...
- проверенные через URL Inspection страницы имеют статус `PASS`, выбранный
  Google canonical совпадает с canonical сайта;
- за последние доступные 28 дней Search Console зафиксировал 1329 показов и 22
  клика, данные появились у 102 URL;
- английские страницы уже участвуют в выдаче, но их CTR пока заметно ниже
  русского, поэтому приоритет — естественные заголовки и практический контент;
- sitemap после каждого контентного релиза отправляется повторно без удаления
  ресурса.
...
Команда читает доступ к доменному ресурсу, sitemap, запросы, страницы, страны и
устройства Search Console, а также landing page, каналы, страны и продуктовые
события GA4. Для `--locale` запросы, страны и устройства Search Console
фильтруются через связанную страницу. В GA4 отдельно выводятся все landing page
и публичные переходы из Organic Search без `/app`; внутренний кабинет не
считается SEO-трафиком.
...
Аудит 16 августа 2026 года, окно 28 дней:

- Яндекс: 59 страниц в поиске, 105 показов и 9 кликов;
- Google: 1329 показов, 22 клика, средняя позиция 16,4;
- Метрика: 30 переходов из поиска против 13 в предыдущем окне;
- продуктовые цели: 10 `platform_start`, при этом органические статьи ещё не
  дали подтверждённого запуска платформы;
- техническая проверка: 138 опубликованных на момент аудита URL отвечали `200`
  с self-canonical; старые сообщения о ссылках без `/` относятся к уже
  исправленным обходам.

После релиза 16 августа русскую выдачу повторно оцениваем через 28 дней,
английскую — через 56 дней. Сравниваем показы, клики, страны, landing page,
`platform_start`, `signup_complete` и основную активацию `first_device_seen`.

В этом релизе добавлены инструкции по установке Zigbee2MQTT на Windows и
Zigbee-клапану полива, потому что они продолжают уже подтверждённые поисковые
темы и не дублируют существующие материалы. Backlog для следующего решения:
...
## Демоферма и обновление продукта

`VITE_DEMO_ENABLED=true` показывает вход в демо; backend отдельно требует
`DEMO_ENABLED=true`. Сначала проверяется и включается backend, затем собирается
сайт с флагом. В CI флаг берётся из repository variable
`VITE_DEMO_ENABLED` (по умолчанию выключен). Демоферма использует общие страницы
кабинета и API, поэтому следующие изменения приложения доступны и в ней.
Гостевая ферма живёт 24 часа; сохранённая связана с аккаунтом. Изоляция и
ограничения описаны в ADR-006 и домене demo.

Статьи собираются через `src/entry-public-server.jsx` из того же React-компонента,
который затем гидратируется в браузере. Текст и CTA доступны без JavaScript,
а переход к интерактивной странице сохраняет DOM статьи.

Воронка демо: `demo_open → demo_ready → demo_action → demo_save`.
Отдельно измеряются `login_view`, `sso_start`, `demo_reset`, `demo_exit`
и `demo_real_setup_start`. Параметр `mode=account|demo` передаётся в события
и просмотры; в GA4 его нужно зарегистрировать как event-scoped custom dimension.
`provider` обозначает выбранный способ входа. События оборудования и
автоматизаций из демо не считаются реальной активацией. `telegram_contact`
означает клик, а не отправленное сообщение.

После публикации сравнить русские и английские органические входы отдельно:
просмотр статьи → открытие демо → первое действие → сохранение → начало
подключения → `first_device_seen` в режиме account. Пять обновлённых тем:
подключение Home Assistant, полив в HA, Zigbee2MQTT на Windows, датчик ESP32 и
Zigbee-клапан. Их URL сохранены; вместо расширения числа похожих статей
уточнены совместимость, проверка результата и следующий шаг к продукту.

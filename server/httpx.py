...
test suite.  This stub is provided because the GitHub Actions
environment used by this repository does not reliably install third
party dependencies during the test phase.  The real ``httpx``
package is used by FastAPI's ``TestClient`` under the hood, but for
our integration tests we only require a minimal asynchronous client
capable of making in‑process requests directly against the FastAPI
application.  To avoid a hard dependency on the external package we
provide a small subset of the ``httpx`` API here.  Only the
functionality used by the tests is implemented; any other use will
raise ``NotImplementedError``.

Classes implemented:

* ``ASGITransport`` – a simple wrapper around the FastAPI application.
  The transport carries the app reference used by the client to route
  requests internally.  It exposes no public methods and is merely a
  container for the ``app`` attribute.
* ``Response`` – a minimal response object exposing ``status_code``
  and a ``json()`` method returning the parsed JSON content.
* ``AsyncClient`` – an asynchronous context manager with ``get``,
  ``post`` and ``put`` methods.  It parses the request path and
  dispatches to the appropriate FastAPI endpoint functions imported
  from ``app.main``.  Database sessions are created via
  ``app.core.database.SessionLocal`` which is monkey‑patched in the
  test module to use an in‑memory SQLite engine.

This stub deliberately omits many aspects of the full ``httpx``
implementation including streaming bodies, cookies, redirects,
middleware and error handling.  If additional behaviour is required
by future tests it can be incrementally added here.
"""

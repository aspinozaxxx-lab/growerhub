...
import ru.growerhub.backend.sensor.SensorHistoryService;
...
public class DeviceIngestionService {
    ...
    public void handleState(String deviceId, DeviceState state, LocalDateTime now) {
        ...
        List<SensorReadingSummary> summaries = sensorHistoryService.record(deviceId, extractMeasurements(state), now);
    ...
    private List<SensorMeasurement> extractMeasurements(DeviceState state) {
        ...
        if (soil != null && soil.ports() != null) {
            for (DeviceState.SoilPort port : soil.ports()) {

...
public class SensorQueryService {
    ...
    public List<SensorView> listByDevice(DeviceEntity device) {
        if (device == null || device.getId() == null) {
            return List.of();
        }
        List<SensorEntity> sensors = sensorRepository.findAllByDevice_Id(device.getId());
        if (sensors.isEmpty()) {
            return List.of();
        }
    ...
    public List<SensorView> listByPlant(PlantEntity plant) {
        if (plant == null || plant.getId() == null) {
            return List.of();
        }
        List<SensorPlantBindingEntity> bindings = bindingRepository.findAllByPlant_Id(plant.getId());
        if (bindings.isEmpty()) {
            return List.of();
        }

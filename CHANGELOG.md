...
- Тест 	est_manual_watering_stop с фейковым MQTT-паблишером (server/tests/test_api_manual_watering_stop.py).
- Теневое хранилище состояний устройств (server/device_shadow.py) с расчётом emaining_s на сервере.
...
- Расчёт emaining_s теперь полностью выполняется на сервере по duration_s и started_at.

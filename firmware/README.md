...
Grovika поднимает локальную точку доступа и web-интерфейс настройки; точка остаётся доступной и во время подключения устройства к внешней сети. В нём показываются полный `device_id`, MQTTS endpoint и состояние подключения, но MQTT-пароль никогда не возвращается.

Список сохранённых сетей объединяет добавленные вручную и встроенные сети без дубликатов. Ручные сети имеют приоритет. Любую сеть можно удалить; после первого изменения сохраняется явный список `wifi.json` версии 2, поэтому удалённая встроенная сеть не возвращается после перезапуска. Старый файл версии 1 мигрирует автоматически без потери добавленных сетей.

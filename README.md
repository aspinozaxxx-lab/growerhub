# 🌱 GrowerHub - Интеллектуальная система ухода за растениями1
![Firmware CI](https://github.com/aspinozaxxx-lab/growerhub/actions/workflows/ci-firmware.yml/badge.svg) 
![Server CI/CD](https://github.com/aspinozaxxx-lab/growerhub/actions/workflows/ci-cd.yml/badge.svg)<br>
![PlatformIO](https://img.shields.io/badge/platformio-ESP32-orange?logo=platformio)
![PlatformIO](https://img.shields.io/badge/platformio-ESP8266-blue?logo=platformio)<br>
![C++](https://img.shields.io/badge/language-C++17-blue?logo=c%2B%2B)
![Python](https://img.shields.io/badge/python-3.11+-blue?logo=python&logoColor=white)
![Lines of code](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/aspinozaxxx-lab/growerhub/main/.github/metrics/loc-badge.json)<br>
![Ansible](https://img.shields.io/badge/ansible-automation-black?logo=ansible)
![FastAPI](https://img.shields.io/badge/fastapi-uvicorn%2Bgunicorn-009688?logo=fastapi)<br>
![Last Commit](https://img.shields.io/github/last-commit/aspinozaxxx-lab/growerhub)
![Coverage](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Faspinozaxxx-lab%2Fgrowerhub%2Fmain%2F.github%2Fmetrics%2Fcoverage-badge.json)




📚 Оглавление

Описание проекта

🚀 Функциональные возможности

🛠️ Аппаратно-программный комплект (MVP)

💻 Программная часть

🏗️ Архитектура

⚙️ Серверная архитектура (Proxmox VE, PostgreSQL, pgAdmin)

📁 Структура проекта

⚡ Быстрый старт

🔧 Системные сервисы

🔒 Переменные окружения

📝 Последние изменения

🎯 Планы развития

📞 Контакты

📝 Описание проекта

GrowerHub — это современная экосистема для автоматизации полива растений с возможностью мониторинга и управления через интуитивно понятный веб-интерфейс.

MVP — это аппаратно-программный комплект, включающий устройство, упаковку и инструкции, необходимые для продажи и быстрого старта пользователя.

🚀 Функциональные возможности

🤖 Автоматический и ручной режимы полива

💡 Управление освещением

📊 Современный веб-интерфейс

🔄 OTA-обновления прошивки

⚡ Авто-деплой серверной части

⚡ Развертывание через Ansible

🆔 Персональный DeviceID на основе MAC-адреса

👤 Личный кабинет (в планах)

📦 Готовый комплект «всё включено»

🛠️ Аппаратно-программный комплект (MVP)

В комплект входят:

устройство в корпусе

насос

реле

датчик влажности почвы

кабели, блок питания

инструкция, коробка

Опции расширения:

управление освещением

аксессуары

💻 Программная часть

Контроллер: ESP32 (PlatformIO, C++)

Сервер: FastAPI + PostgreSQL

Веб-интерфейс: мониторинг устройств

CI/CD: GitHub Actions

🏗️ Архитектура
ESP32

DeviceID на основе MAC

пины: 34 — почва, 4 — полив, 5 — свет, 15 — DHT22

RTC, OTA, EEPROM, 2 реле

⚙️ Серверная архитектура (Proxmox VE, PostgreSQL, pgAdmin)

Проект развернут на Proxmox VE, где сервисы вынесены на отдельные ВМ.
Часть ролей уже отделена: база данных и pgAdmin работают на отдельных машинах.

Виртуальные машины
ВМ	IP	Группы	Назначение
growerhub	192.168.0.11	app	FastAPI backend / служебный сервер
ansible-host	192.168.0.80	ansible_host	Контроллер Ansible
gh-db	192.168.0.81	gh_db	PostgreSQL
gh-app	192.168.0.82	gh_app	Будущий app-сервер
gh-nginx	192.168.0.83	gh_nginx	Nginx / reverse proxy
gh-mqtt	192.168.0.84	gh_mqtt	MQTT брокер
gh-tools	192.168.0.85	gh_tools	pgAdmin и инструменты
PostgreSQL (gh-db)

Хост: 192.168.0.81

Порт: 5432

БД: gh_db

Пользователь: db-admin

Trusted clients: 192.168.0.11, 192.168.0.85

Плейбук: playbooks/gh_db_install.yml

Роль: roles/gh_db_postgresql

Данные для подключения
Name: gh-db
Host: 192.168.0.81
Port: 5432
Maintenance DB: gh_db
Username: db-admin
Password: gh_db_admin_dev_1234


⚠️ Пароли нужно перенести в Ansible Vault

pgAdmin (gh-tools)

URL: http://192.168.0.85:5050

Email: admin@growerhub.dev

Плейбук: playbooks/gh_tools_pgadmin.yml

Роль: roles/pgadmin

pgAdmin автоматически подгружает подключение к базе из servers.json внутри контейнера.

📁 Структура проекта
growerhub/
├── firmware/
├── server/
├── ansible/
├── web/
├── docs/
├── scripts/
├── deploy.sh
└── deploy_agent.py

⚡ Быстрый старт
# Загрузка прошивки
cd firmware && pio run --target upload

# Проверка сервисов
sudo systemctl status growerhub growerhub-deploy-agent

🔧 Системные сервисы

growerhub.service

growerhub-deploy-agent.service

⚙️ Запуск плейбуков Ansible
ansible-playbook -i inventory/hosts.ini playbooks/gh_tools_pgadmin.yml


(Аналогично запускаются все роли.)

🔒 Переменные окружения
sudo systemctl edit watering-deploy-agent.service

[Service]
Environment=GITHUB_TOKEN="secret token"

📝 Последние изменения

(оставлено как было)

🎯 Планы развития

(оставлено как было)

📞 Контакты

Сервер: http://192.168.0.11

Веб: https://growerhub.ru

Репозиторий: https://github.com/aspinozaxxx-lab/growerhub